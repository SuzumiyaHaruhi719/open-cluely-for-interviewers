...
import { BALANCED_AUTO_GATE, config as serverConfig } from './config';
...
const MONITOR_LIVENESS_WAITS = BALANCED_AUTO_GATE.livenessWaits;
const MONITOR_LIVENESS_CHARS = BALANCED_AUTO_GATE.livenessChars;
  ...
  async function evaluate(text: string, anchorSeq?: number): Promise<void> {
    ...
    try {
      ...
      if (!decision.shouldGenerate) {
        consecutiveMonitorWaits += 1;
        if (
          consecutiveMonitorWaits >= MONITOR_LIVENESS_WAITS &&
          candidateWindow.length >= MONITOR_LIVENESS_CHARS
        ) {
          decision = {
            shouldGenerate: true,
            reason: '持续回答已形成，但监控连续等待，需要专家检查最高价值的证据缺口',
            focusHint:
              '候选人已形成持续的实质性回答，但仍缺少可验证的个人行动、关键取舍或结果；只选择与当前回答最相关且信息增益最高的一项追问',
            urgency: 'med',
