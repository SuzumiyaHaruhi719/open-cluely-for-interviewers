# open-cluely — project context for Claude Code
...
The vault root (`Documents\Obsidian\WTATC\`) is its own git repo pushing to `github.com:SuzumiyaHaruhi719/Obsidian`. Every Claude session-end auto-pushes the vault, so writes to `Implementation/` propagate to GitHub without manual intervention.
